...
20. **(2026-08-24) `provider-auth-failed` persistence branch.** Observed incident: an ElevenLabs 401 burst fired the rule, then it *auto-resolved while the key was still broken* — the 5-min window went empty once the breaker opened and traffic stopped, and "lack of data" looked like "recovered". The rule now has a second, persistent branch: it also fires while the provider's **most recent `provider_call_logs` row** (any operation, including probe pings; 24 h lookback, `LAST_SIGNAL_LOOKBACK_MS`) is an auth failure, and resolves only once a call or probe from the provider succeeds. Implemented via a new `queryProviderLastSignals` data provider (`SELECT DISTINCT ON (provider_id) … ORDER BY provider_id, created_at DESC, id DESC` — served by the existing `(provider_id, created_at)` index, no migration), its own try/catch isolation (finding 19's intent), and inclusion of last-signal providers in both the name-lookup id set and `perProviderScopeParts` (an old-signal-only provider must still be evaluated). The windowed count branch is unchanged and takes message precedence when both are met. The `maxUnresolvedHours: 6` safety valve bounds a silent provider: the alert re-fires while the last signal is <24 h old, then settles — `provider-down` (probe branch) remains the persistent guardrail in that case. Note: this bug was reachable even with probes ENABLED — the default probe cadence (`probeSettings.cooldownMinutes: 10`) is SLOWER than the rule window (5 min), so probe rows alone could not keep the window filled and the alert flapped fire→resolve→fire on the probe cadence. Unit: 4 new tests (persist-while-last-signal-auth, resolve-on-success, non-auth last signal quiet, count-message precedence); e2e: stale (30-min-old) auth row outside the window → firing via the last-signal branch → success row → auto-resolve.
21. **(2026-08-24) `provider-down` breaker branch wired to the live registry.** Audit of all 21 rules for the "silence masquerading as recovery" bug class found that `getBreakers` was still the Phase-2 empty-map stub (P3-01 never replaced it), so the breaker-OPEN branch of `provider-down` — and the breaker note in `provider-auth-failed`'s persistence message — never fired in production. Now wired via the injected `CircuitBreakerRegistry.openProviderIds()`; see P3-01 Implementation note 1 for the regression test + the app-world/test-world container split gotcha.
