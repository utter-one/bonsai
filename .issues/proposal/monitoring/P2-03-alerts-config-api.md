...
## Addendum (2026-08-19) — rule catalog endpoint `GET /api/monitoring/rules`

The config API made the gap visible: `PUT /config` accepts `rules` keyed by rule id, but the 20 valid ids, their default parameters, and their threshold semantics were discoverable **only from source** (`AlertEvents.ts`) — not in Swagger, not in any response. The Console config UI (and any API consumer) cannot render a rule editor from the existing surface. This addendum closes that with a 12th monitoring route.

**Design decisions:**

1. **Single source of truth = the engine rule registry.** `AlertEvents.ts` gains `buildRuleCatalog(): AlertRuleCatalogItem[]` — a projection of `DEFAULT_RULES` that strips the `evaluate` closures. The served catalog is *derived*, so it cannot drift from the evaluators or from `RULE_IDS` (which validates config keys).
2. **`summary` field on `AlertRuleDef`** (required): a one-line condition description including threshold semantics ("100% of calls failed… or ≥ 3 consecutive probe failures"). Filled for all 20 rules; the per-rule threshold comments in source are folded into these summaries. This is the only P2-01-module change the addendum makes — the engine ignores no fields, adds one.
3. **`defaultParams` reuses the engine's `ruleParamsSchema`** in the response contract (`alertRuleCatalogItemSchema`) — no parallel field definitions to drift. The contract already imported `RULE_IDS` from `AlertEvents` (finding 19's superRefine), so the contract→service import direction for the rule registry is established house practice for this file.
4. **Static, unpaginated, no query params** — 20 items of a few KB; `getRuleCatalog()` is synchronous (no DB) but still `requirePermission(context, SYSTEM_MONITORING)` (defense in depth, house pattern).
5. **Schemas**: `alertRuleCatalogItemSchema` (`id`, `scope`, `severity`, `summary`, `defaultParams`) + `alertRuleCatalogResponseSchema` (`{ rules: [...] }`), `.openapi()` named — Swagger picks them up automatically; `PUT /config` is now self-documenting against this catalog.
6. **Tests**: three in this suite (exact 20-id set pin — adding a rule on purpose must update the test; per-item shape + no closure/internal leak; sampled engine-default spot checks incl. `provider-down` 3/10/5/2 and `high-memory` 1536 MB bytes, plus global/per_provider 8/12 split). The P2-04 `ROUTES` matrix gained the 12th route (200/403/401 covered by the existing loop; test titles now interpolate `ROUTES.length`).

**Files**: `src/services/monitoring/AlertEvents.ts` (summary ×20, `buildRuleCatalog`), `src/http/contracts/monitoring.ts` (2 schemas + types, `ruleParamsSchema` import), `src/services/monitoring/MonitoringService.ts` (`getRuleCatalog`), `src/http/controllers/MonitoringController.ts` (route + OpenAPI path), `tests/e2e/monitoring-alerts-config.test.ts` (+3 tests), `tests/e2e/monitoring-rbac.test.ts` (matrix +1).
