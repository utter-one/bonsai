...
- **Hourly** (node-cron `0 * * * *`): roll up the previous complete hour of `provider_call_logs` → `provider_call_stats_hourly` via `INSERT ... SELECT` using `percentile_cont` for p50/p95/p99 TTFT + p95 duration; `stalled_count` = rows with `metrics->>'maxChunkGapMs' > 10000`; `rtf_over_1_count` = TTS rows with `metrics->>'audioDurationMs' > 0 AND duration_ms > metrics->>'audioDurationMs'` — the variant fields are extracted from the `metrics` jsonb (camelCase keys per the TS `CallMetrics` type, e.g. `metrics->>'maxChunkGapMs'::int`; `percentile_cont` skips missing keys as NULL). `ON CONFLICT DO NOTHING` for idempotency.
