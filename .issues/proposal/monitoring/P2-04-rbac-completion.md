...
2. **The 11 routes (verified in `MonitoringController.registerRoutes`)**: `GET /health`, `GET /health/history`, `GET /providers`, `GET /provider-calls`, `GET /provider-stats`, `GET /metrics`, `GET /alerts`, `GET /alerts/:id`, `POST /alerts/:id/acknowledge`, `GET /config`, `PUT /config`.
...
4. **401 vs 403 ordering**: `checkPermissions` throws `UnauthorizedError` *before* the permission check, so unauthenticated requests get 401 on all 11 routes, never 403.
...
- **E2E:** `tests/e2e/monitoring-rbac.test.ts` (8 tests, implemented 2026-08-19):
  ...
  2. super_admin → 200 on all 11 routes (provider-stats/metrics supplied their required query windows; alert row seeded per test).
  3–6. developer / content_manager / support / viewer → 403 on all 11 routes.
  7. Unauthenticated → 401 on all 11 routes.
