...
| 12 | **Metric catalog endpoint** `GET /api/monitoring/metric-catalog` — addendum so UIs can build dashboards from the live metric catalog (name/kind/description/buckets/effective maxSeries) instead of a hardcoded metric list; served from the closed MetricsRegistry config so it cannot drift. | P1-08 addendum (2026-09-01) |
