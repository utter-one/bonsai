...
## Addendum (2026-09-01) — metric catalog endpoint `GET /api/monitoring/metric-catalog`

The metrics series endpoint (`/api/monitoring/metrics`) requires `name` — but the 35 valid names, their kinds, histogram buckets and cardinality caps were discoverable **only from source** (`MetricsRegistry.ts`). The Console had to hardcode its metric list, and it was already drifting (its constant predated `event_loop_lag_max_ms` and the two 2026-09-01 health-check metrics). This addendum closes that with a 7th read-only route, following the P2-03 rule-catalog precedent.

**Design decisions:**

1. **Single source of truth = the closed registry config.** `MetricsRegistry.ts` gains `buildMetricCatalog(): MetricCatalogEntry[]` — a pure projection of `METRIC_CONFIGS` + `METRIC_DESCRIPTIONS` (the same maps the registry enforces and the Prometheus exporter renders). The served catalog is *derived*, so it cannot drift from the series endpoint or the exporter.
2. **Effective caps.** `maxSeries` is the *effective* cap (`config.maxSeries ?? DEFAULT_MAX_SERIES`, default 50) — the config's optional field is resolved so consumers do not need to know the registry default.
3. **Static, unpaginated, no query params** — 35 items of a few KB; `getMetricCatalog()` is synchronous (no DB) but still `requirePermission(context, SYSTEM_MONITORING)` (defense in depth, house pattern).
4. **Schemas**: `metricCatalogItemSchema` (`name`, `kind`, `description`, `buckets?`, `maxSeries`) + `metricCatalogResponseSchema` (`{ metrics: [...] }`), `.openapi()`-named — Swagger picks them up automatically. `buckets` is optional (histograms only) and the projection omits the key entirely for counters/gauges.
5. **Tests**: five unit tests in `tests/unit/monitoring/metricsRegistry.test.ts` (exact 35-name set + sort; kind/description/effective-maxSeries projection for every metric; buckets on histograms only, ascending, no extra keys; cap and bucket spot checks), three in `tests/e2e/monitoring-endpoints.test.ts` (exact name-set pin; per-item shape + exact key set; config spot checks incl. the two health-check metrics), and the route joins the P2-04 `ROUTES` matrix (200/403/401 covered by the existing loops).

**Files**: `src/services/monitoring/MetricsRegistry.ts` (`buildMetricCatalog`, `MetricCatalogEntry`, exported `DEFAULT_MAX_SERIES`), `src/http/contracts/monitoring.ts` (2 schemas + types), `src/services/monitoring/MonitoringService.ts` (`getMetricCatalog`), `src/http/controllers/MonitoringController.ts` (route + OpenAPI path), `tests/unit/monitoring/metricsRegistry.test.ts` (+5), `tests/e2e/monitoring-endpoints.test.ts` (+3, route in the RBAC loop), `tests/e2e/monitoring-rbac.test.ts` (matrix +1), `docs/guide/monitoring-api.md` (§4.15).
