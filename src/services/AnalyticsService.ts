  ...
  async getLatencyStats(projectId: string, query: AnalyticsQuery, context: RequestContext): Promise<LatencyStatsResponse> {
    ...
    const result = await db.execute(sql.raw(`
      SELECT
        ...
        ${this.buildMetricAggSQL('stageTransitionDurationMs', true)},
    ...
    for (const row of result.rows as any[]) {
      ...
      if (row.role === 'user') {
        ...
        turnMap.set(turnIndex, {
          ...
          stageTransitionStartMs: prev.stageTransitionStartMs ?? null,
          stageTransitionEndMs: prev.stageTransitionEndMs ?? null,
          stageTransitionDurationMs: prev.stageTransitionDurationMs ?? null,
