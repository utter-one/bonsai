...
export class ContextTransformerExecutor {
  ...
  private async executeTransformer(session: Session, transformerData: TransformerRuntimeData, context: ConversationContext): Promise<TransformerExecutionResult> {
    ...
    try {
      ...
      return { transformerId: transformer.id, transformerName: transformer.name, appliedFields, parsedValues, renderedPrompt, rawResponse, llmUsage: result.usage, durationMs: endMs - startMs, startMs, endMs };
    } catch (error) {
