...
export class PersonaService extends BaseService {
   ...
   * @param input - Persona creation data including id, name, description, prompt, ttsProviderId, voiceConfig, and optional metadata
  ...
  async createPersona(input: CreatePersonaRequest, context: RequestContext): Promise<PersonaResponse> {
    ...
    try {
      const persona = await db.insert(personas).values({ id: personaId, projectId: input.projectId, name: input.name, description: input.description ?? null, prompt: input.prompt, ttsProviderId: input.ttsProviderId, voiceConfig: input.voiceConfig, metadata: input.metadata, version: 1 }).returning();

      const createdPersona = persona[0];

      await this.auditService.logCreate('persona', createdPersona.id, { id: createdPersona.id, projectId: createdPersona.projectId, name: createdPersona.name, description: createdPersona.description, prompt: createdPersona.prompt, ttsProviderId: createdPersona.ttsProviderId, voiceConfig: createdPersona.voiceConfig, metadata: createdPersona.metadata }, context?.adminId);
  ...
  async updatePersona(id: string, input: UpdatePersonaRequest, context: RequestContext): Promise<PersonaResponse> {
    ...
    try {
      ...
      await this.auditService.logUpdate('persona', persona.id, { id: existingPersona.id, name: existingPersona.name, description: existingPersona.description, prompt: existingPersona.prompt, ttsProviderId: existingPersona.ttsProviderId, voiceConfig: existingPersona.voiceConfig, metadata: existingPersona.metadata }, { id: persona.id, name: persona.name, description: persona.description, prompt: persona.prompt, ttsProviderId: persona.ttsProviderId, voiceConfig: persona.voiceConfig, metadata: persona.metadata }, context?.adminId);
  ...
  async deletePersona(id: string, expectedVersion: number, context: RequestContext): Promise<void> {
    ...
    try {
      ...
      await this.auditService.logDelete('persona', id, { id: existingPersona.id, name: existingPersona.name, description: existingPersona.description, prompt: existingPersona.prompt, ttsProviderId: existingPersona.ttsProviderId, voiceConfig: existingPersona.voiceConfig, metadata: existingPersona.metadata }, context?.adminId);
